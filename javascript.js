(function() {
  ...
  function showMenuList() {
    ...
    menu.forEach(function(item, index) {
      ...
      if (slot_balance.value < item.price || broken_menu.indexOf(item.id) >= 0) {
        string += '<li disabled=disabled>';
      } else {
        string += '<li>'
      }
      // html += '<h3>' + item.id + '. ' + name + '</h3><span>' + item.price + '</span></li>';

      string += '<h3>{id}. {name}</h3><span>{price}</span></li>';
      html += createTemplate(string, {'id': item.id, 'name': name, 'price': item.price});
    });
  ...
  function showCashList() {
    ...
    unit.forEach(function(item, index) {
      var string = '';
      string += '<li><button type="button" value="{index}">{item}원</button><span>{amount}개</span></li>';
      html += createTemplate(string, {'index': index, 'item': item, 'amount': wallet[index]});
