...
* 여기서는 wallet과 products가 있다.
## View
* 사용자가 볼 결과물을 생성하기 위해 모델로부터 정보를 얻어 온다.
* 여기서는 wallet과 products 정보를 얻어와서 product-list와 console, totalMoney, indicatorMoney, moneyList, buttonList,amountList를 렌더링한다.
## Controller
* 모델에 명령을 보냄으로써 모델의 상태를 변경할 수 있다. (예: 워드 프로세서에서 문서를 편집하는 것) 또, 컨트롤러가 관련된 뷰에 명령을 보냄으로써 모델의 표시 방법을 바꿀 수 있다. (문서를 스크롤하는 것)
* 여기서는 돈이 인출됬을경우, 상품을 누를 경우에 모델에 명령을 보냄으로써 Wallet과 products의 상태를 변경한다.